...
App init_handler() {
  ...
  app.window =
      SDL_CreateWindow("tetriC", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
...
void draw_rect(App app, u_int8_t x, u_int8_t y, uint32_t color, bool fancy) {
  const int padding_x = BLOCK_SIZE;
  const int padding_y = BLOCK_SIZE;
  SDL_Rect r;
  ...
  if (texture == NULL) {
    ...
    texture =
        SDL_CreateTexture(app.renderer, SDL_PIXELFORMAT_ARGB8888,
  ...
  if (surface == NULL) {
    ...
    surface = SDL_CreateRGBSurfaceFrom(NULL, BLOCK_SIZE, BLOCK_SIZE, 32, 0,
  ...
  for (u_int32_t i = 0; i < BLOCK_SIZE; i++) {
    for (u_int32_t j = 0; j < BLOCK_SIZE; j++) {
      if (i == 0 || j == 0 || i == BLOCK_SIZE - 1 || j == BLOCK_SIZE - 1) {
        ...
        if (fancy) {
          ((uint32_t *)(surface->pixels))[i * BLOCK_SIZE + j] =
              color * ((abs(BLOCK_SIZE / 2 - i) + abs(BLOCK_SIZE / 2 - j))) *
              10 / BLOCK_SIZE;
        } else {
