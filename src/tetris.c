...
const Color initial_shape_list[7] = {L, O, J, S, I, T, Z};
...
const uint32_t color_hex[7] = {
    0xff7f00, 0x00ff00, 0xff0000, 0x800080, 0x00ffff, 0xffff00, 0x0000ff,
};
  ...
  for (uint8_t i = 0; i < SHAPE_FIELD_SIZE; i++) {
    for (uint8_t j = 0; j < SHAPE_FIELD_SIZE; j++) {
      int rotations[] = {
          ...
          (SHAPE_FIELD_SIZE - i - 1) * SHAPE_FIELD_SIZE +
              (SHAPE_FIELD_SIZE - j - 1),
...
void render_field(GameState *game_state) {
  uint32_t field_padding_x = 1;
  uint32_t field_padding_y = 1;
  for (uint8_t i = 4; i < 24; i++) {
    for (uint8_t j = 0; j < 10; j++) {
      if (game_state->field[i][j] != 0) {
        draw_rect(game_state->app, j + field_padding_x, i + field_padding_y - 4,
                  color_hex[game_state->field[i][j] - 1], false);
      } else {
        draw_rect(game_state->app, j + field_padding_x, i + field_padding_y - 4,
                  0x101010, true);
      }
  ...
  for (uint8_t i = 0; i < SHAPE_FIELD_SIZE; i++) {
    for (uint8_t j = 0; j < SHAPE_FIELD_SIZE; j++) {
      ...
      if (shapes[game_state->piece.color - 1]
                [rotations[game_state->piece.rotation]] != ' ') {
        draw_rect(game_state->app, game_state->piece.x + j + field_padding_x,
                  game_state->piece.y + i + field_padding_y - 4,
                  color_hex[game_state->piece.color - 1], false);
      }
    }
  }
  for (uint8_t i = 0; i < 20; i++) {
    draw_rect(game_state->app, field_padding_x - 1, i + field_padding_y,
              0x505050, true);
    draw_rect(game_state->app, field_padding_x + 10, i + field_padding_y,
              0x505050, true);
  }
  for (uint8_t j = 0; j < 12; j++) {
    draw_rect(game_state->app, j + field_padding_x - 1, field_padding_y - 1,
              0x505050, true);
    draw_rect(game_state->app, j + field_padding_x - 1, field_padding_y + 20,
              0x505050, true);
  }
...
bool can_go_to(GameState *game_state, PieceState piece_state, int8_t delta_x,
               int8_t delta_y, int8_t delta_rotation) {
  for (uint8_t i = 0; i < SHAPE_FIELD_SIZE; i++) {
    for (uint8_t j = 0; j < SHAPE_FIELD_SIZE; j++) {
      int rotations[] = {
          i * SHAPE_FIELD_SIZE + j,
      ...
      if (shapes[piece_state.color - 1]
                [rotations[(piece_state.rotation + delta_rotation) % 4]] !=
          ' ') {
        if (piece_state.y + i + delta_y >= 24 ||
            piece_state.y + i + delta_y < 0 ||
            piece_state.x + j + delta_x < 0 ||
            piece_state.x + j + delta_x >= 10 ||
            game_state->field[piece_state.y + i + delta_y]
                             [piece_state.x + j + delta_x] != 0) {
          return false;
        }
      }
...
void spawn_shape(GameState *game_state) {
  ...
  PieceState new_piece = {5, 0, 0,
                          game_state->piece_list[game_state->piece_index]};
...
void rotate_shape(GameState *game_state) {
  for (uint8_t i = 0; i < OFFSET_COUNTS; i++) {
    if (can_go_to(game_state, game_state->piece,
                  shape_rotate_offsets[game_state->piece.color - 1][i]
                                      [game_state->piece.rotation][0],
                  shape_rotate_offsets[game_state->piece.color - 1][i]
                                      [game_state->piece.rotation][1],
                  1)) {
      game_state->piece.x +=
          shape_rotate_offsets[game_state->piece.color - 1][i]
                              [game_state->piece.rotation][0];
...
void generate_sound(void *user_data, uint8_t *stream, int len) {
  ...
  const int notes[] = {
      O2_LA,  O2_LA,  O2_LA,  SILENT,

      O1_MI,  O1_MI,

      O1_FA,  O1_FA,

      O2_SOL, O2_SOL, O2_SOL, SILENT,

      O1_FA,  O1_FA,

      O1_MI,  O1_MI,

      O1_RE,  O1_RE,  O1_RE,  SILENT,
      ...
      O1_FA,  O1_FA,  O1_FA,  SILENT,

      O1_RE,  O1_RE,  O1_RE,  SILENT,

      O1_RE,  O1_RE,  O1_RE,  O1_RE,  SILENT, SILENT, SILENT, SILENT,

      O2_SOL, O2_SOL, O2_SOL, SILENT,

      O2_SIb, O2_SIb,

      O2_RE,  O2_RE,  O2_RE,  SILENT,

      O2_DO,  O2_DO,

      O2_SIb, O2_SIb, O2_SIb, SILENT,
...
int main(int argc, char const *argv[]) {
  ...
  srand(time(0));
  GameState game_state = {{0}, init_handler(), {0, 0, 0, 0}, {0},
                          -1,  false,          100};
  spawn_shape(&game_state);
